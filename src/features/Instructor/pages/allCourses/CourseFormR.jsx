...
function reducer(state, action) {
  switch (action.type) {
    case "slugg":
      return { ...state, slugg: action.payload }; // return new state
    default:
      return state;
  }
}
...
const CourseFormR = () => {
  ...
  const [state, dispatch] = useReducer(reducer, { slugg: "" });
        ...
        <div className="w-full p-10 bg-gray-400">
          <form onSubmit={handleSubmit} className="bg-white p-10 rounded-xl">
            ...
            <div className="flex gap-10">
              ...
              <div className="w-1/2 flex flex-col  mb-5">
                <label className="text-gray-600 text-sm">Slugg</label>
                <input
                  className=" border border-gray-500 p-2 w-full rounded-md focus:border-logoColor2 outline-none"
                  type="text"
                  value={state.slugg}
                  onChange={(e) =>
                    dispatch({ type: "slugg", payload: e.target.value })
                  }
                  required
                />
              </div>
